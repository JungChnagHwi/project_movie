<template>
    <div>
      <div class="card recommend-container">
        ...
        <div class="card-list px-3" ref="cardList">
          <card class="cardrev" v-for="movie in movies" :key="movie.id" @click="openModal(movie)">
            ...
            <button @click="toggleLike(movie)" v-if="!movie.isLiked" style="display: none;">좋아요</button>
            <button @click="toggleLike(movie)" v-if="movie.isLiked" style="display: none;">좋아요 취소</button>
          </card>
        </div>
      ...
      <div id="commingsoonModal" class="modal" @click="closeModalOutside">
          ...
          <div v-if="selectedMovie" class="movie-info">
            ...
            <div class="movie-details">
              ...
              <div class="overview-rating">
                ...
                <!-- <button @click="toggleLike(selectedMovie)" v-if="!selectedMovie.isLiked">좋아요</button>
                <button @click="toggleLike(selectedMovie)" v-if="selectedMovie.isLiked">좋아요 취소</button> -->
              </div>
            </div>
            <br>
  ...
  const getMovies = function () {
    axios({
      method: 'GET',
      url: `http://127.0.0.1:8000/movies/commingsoon/`,
    })
    .then((res) => {
      movies.value = res.data.map(movie => ({ ...movie, isLiked: false })); // 각 영화에 대한 isLiked 속성 추가
    })
    .catch((err) => {
      console.log(err);
    });
  ...
  function openModal(movie) {
    selectedMovie.value = { ...movie }; // 각 영화 정보 복사
    document.getElementById('commingsoonModal').style.display = 'block';
  }
  
  function closeModal() {
    selectedMovie.value = null;
    document.getElementById('commingsoonModal').style.display = 'none';
  }
  
  function closeModalOutside(event) {
    if (event.target === document.getElementById('commingsoonModal')) {
      closeModal();
    }
  }
  ...
  const allMovies = async () => {
    try {
      ...
      allmovies.value = res.data.map(movie => ({ ...movie, isLiked: false })); // 각 영화에 대한 isLiked 속성 추가
    } catch (error) {
