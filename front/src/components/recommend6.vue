  ...
  <div>
    <div class="card recommend-container">
      ...
      <div style="display: flex; align-items: center; text-align: center;">
        <select id="companySelect" v-model="selectedCompany" @change="filterByCompany" class="form-select language-select" style=" margin-right: 0;">
    ...
    <div id="companyModal" class="modal" @click="closeModalOutside">
...
const selectedcompany = ref('Walt Disney Pictures')
const cardList = ref(null);

const getMovies = function () {
  axios({
    method: 'GET',
    url: `http://127.0.0.1:8000/movies/company/`,
    params: {
...
const extractCompanies = () => {
  ...
  allmovies.value.forEach(movie => {
    if (movie.companies) {
      const movieCompanies = movie.companies.split(',');
      movieCompanies.forEach(company => companiesSet.add(company.trim()));
    }
  });
  return Array.from(companiesSet);
};

// 추가된 부분: 중복을 제거한 회사 목록
const uniqueCompanies = computed(() => extractCompanies());

// 추가된 부분: 회사 선택이 변경되면 검색 결과를 필터링
const filterByCompany = () => {
  if (selectedCompany.value) {
    const filteredMovies = allmovies.value.filter(movie => {
      if (movie.companies) {
        const movieCompanies = movie.companies.split(',').map(company => company.trim());
        return movieCompanies.includes(selectedCompany.value);
      }
      return false;
    });
